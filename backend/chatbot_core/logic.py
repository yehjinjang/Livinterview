...
from .prompts import (
    check_prompt, question_check_prompt, summary_prompt,
    controlnet_prompt, extract_proposal_prompt, check_completion_prompt,
    followup_question_prompt, furniture_warning_prompt, check_partial_agreement_prompt
)

furniture_keywords = [
    "침대", "책상", "의자", "옷장", "선반", "테이블", "소파",
    "러그", "조명", "커튼", "수납장", "소파베드", "협탁", "서랍", "거울"
]
...
def run_conversation(image_url: str):
    ...
    while True:
        ...
        matched = [
            ...
            if "YES" in gpt.invoke(check_partial_agreement_prompt.format(gpt_response=s, text=user_text)).content.upper()
        ]
        if matched:
        ...
        if is_done:
            ...
            if check_result == "YES":
                ...
                is_confirm = "YES" in gpt.invoke(
                    check_partial_agreement_prompt.format(gpt_response="이미지 생성", text=confirm)
                ).content.upper()
        ...
        if count_unique_furniture_mentions(full_conversation) >= 6:
            warning_msg = gpt.invoke(furniture_warning_prompt.format(conversation=full_conversation)).content
            print("\nGPT 응답:\n", warning_msg)
